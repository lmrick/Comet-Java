...
public class MySQLFastFoodRepository extends MySQLRepository {
	...
	public void loadPlayerData(final FastFoodGameSession gameSession) {
		...
		if (gameSession.getParachutes() == -1) {
			insert("INSERT into fastfood_user_data (player_id, shields, parachutes, missiles, games_played) VALUES(?, 10, 10, 10, 0);", (k) -> {
			}, gameSession.getPlayerId());
	...
	public void savePlayerData(final FastFoodGameSession gameSession) {
		update("UPDATE fastfood_user_data SET parachutes = ?, objects = ?, shields = ?, games_played = ? WHERE player_id = ?", gameSession.getParachutes(), gameSession.getMissiles(), gameSession.getShields(), 1, gameSession.getPlayerId());
	}
