...
public class BundleDao {
	...
	public static void loadActiveBundles(Map<String, RoomBundle> bundles) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("SELECT * FROM room_bundles WHERE enabled = '1'", sqlConnection);
			...
			while (resultSet.next()) {
				...
				try {
					...
					final CustomFloorMapData roomModelData = JsonUtil.getInstance().fromJson(resultSet.getString("model_data"), CustomFloorMapData.class);
					final List<RoomBundleItem> bundleItems = JsonUtil.getInstance().fromJson(resultSet.getString("bundle_data"), new TypeToken<ArrayList<RoomBundleItem>>() {}.getType());
	...
	public static void saveBundle(RoomBundle bundle) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("REPLACE into room_bundles (alias, room_id, model_data, bundle_data, room_config) VALUES(?, ?, ?, ?, ?);", sqlConnection, true);
