...
public class PlayerDao {
	...
	public static PlayerData getDataByUsername(String username) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("SELECT id, motto, figure, gender, email, rank, credits, vip_points, activity_points, seasonal_points, reg_date, last_online, vip, achievement_points, reg_timestamp, favourite_group, last_ip, quest_id, time_muted, name_colour FROM players WHERE username = ?", sqlConnection);
			preparedStatement.setString(1, username);
			
			resultSet = preparedStatement.executeQuery();
			
			while (resultSet.next()) {
				return new PlayerData(resultSet.getInt("id"), username, resultSet.getString("motto"), resultSet.getString("figure"), resultSet.getString("gender"), resultSet.getString("email") == null ? "" : resultSet.getString("email"), resultSet.getInt("rank"), resultSet.getInt("credits"), resultSet.getInt("vip_points"), resultSet.getInt("activity_points"), resultSet.getInt("seasonal_points"), resultSet.getString("reg_date"), resultSet.getInt("last_online"), resultSet.getString("vip").equals("1"), resultSet.getInt("achievement_points"), resultSet.getInt("reg_timestamp"), resultSet.getInt("favourite_group"), resultSet.getString("last_ip"), resultSet.getInt("quest_id"), resultSet.getInt("time_muted"), resultSet.getString("name_colour"), null);
			}
		} catch (SQLException e) {
			SQLUtility.handleSqlException(e);
		} finally {
	...
	public static PlayerData getDataById(int id) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("SELECT id, username, motto, figure, gender, email, rank, credits, vip_points, activity_points, seasonal_points, reg_date, last_online, vip, name_colour, achievement_points, reg_timestamp, favourite_group, last_ip, quest_id, time_muted, name_colour FROM players WHERE id = ?", sqlConnection);
	...
	public static IPlayerAvatar getAvatarById(int id, byte mode) {
		Connection sqlConnection = null;
		PreparedStatement preparedStatement = null;
		ResultSet resultSet = null;
		...
		try {
			...
			while (resultSet.next()) {
				final IPlayerAvatar playerAvatar = new PlayerAvatarData(id, resultSet.getString("username"), resultSet.getString("figure"), resultSet.getString("gender"), resultSet.getString("motto"), resultSet.getInt("reg_timestamp"));
				
				if (mode == IPlayerAvatar.USERNAME_FIGURE_MOTTO) {
	...
	public static PlayerSettings getSettingsById(int id) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("SELECT * FROM player_settings WHERE player_id = ? LIMIT 1;", sqlConnection);
			preparedStatement.setInt(1, id);
	...
	public static PlayerStatistics getStatisticsById(int id) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("SELECT * FROM player_stats WHERE player_id = ? LIMIT 1;", sqlConnection);
			preparedStatement.setInt(1, id);
	...
	public static void updatePlayerStatus(Player player, boolean online, boolean setLastOnline) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("UPDATE players SET online = ?" + (setLastOnline ? ", last_online = ?, last_ip = ?" : "") + " WHERE id = ?", sqlConnection);
	...
	public static String getUsernameByPlayerId(int playerId) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("SELECT `username` FROM players WHERE `id` = ?", sqlConnection);
			preparedStatement.setInt(1, playerId);
	...
	public static int getIdByUsername(String username) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("SELECT `id` FROM players WHERE `username` = ?", sqlConnection);
			preparedStatement.setString(1, username);
	...
	public static String getIpAddress(int playerId) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("SELECT `last_ip` FROM players WHERE `id` = ?", sqlConnection);
			preparedStatement.setInt(1, playerId);
	...
	public static void updatePlayerData(int id, String username, String motto, String figure, int credits, int points, String gender, int favouriteGroup, int activityPoints, int seasonalPoints, int questId, int achievementPoints, String nameColour) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("UPDATE players SET username = ?, motto = ?, figure = ?, credits = ?, vip_points = ?, gender = ?, favourite_group = ?, activity_points = ?, seasonal_points = ?, quest_id = ?, achievement_points = ?, name_colour = ? WHERE id = ?", sqlConnection);
			preparedStatement.setString(1, username);
	...
	public static boolean updatePlayerStatistics(PlayerStatistics playerStatistics) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("UPDATE player_stats SET achievement_score = ?, total_respect_points = ?, daily_respects = ?, help_tickets = ?, help_tickets_abusive = ?, cautions = ?, bans = ?, daily_scratches = ? WHERE player_id = ?", sqlConnection);
	...
	public static void updateHomeRoom(int homeRoom, int userId) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("UPDATE player_settings SET home_room = ? WHERE player_id = ?", sqlConnection);
	...
	public static void saveWardrobe(String wardrobeData, int userId) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("UPDATE player_settings SET wardrobe = ? WHERE player_id = ?", sqlConnection);
	...
	public static void saveVolume(String volumeData, int userId) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("UPDATE player_settings SET volume = ? WHERE player_id = ?", sqlConnection);
	...
	public static void saveChatStyle(boolean useOldChat, int userId) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("UPDATE player_settings SET chat_oldstyle = ? WHERE player_id = ?", sqlConnection);
	...
	public static void saveIgnoreInvitations(boolean ignoreInvitations, int userId) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("UPDATE player_settings SET ignore_invites = ? WHERE player_id = ?", sqlConnection);
	...
	public static void saveAllowFriendRequests(boolean allowFriendRequests, int userId) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("UPDATE player_settings SET allow_friend_requests = ? WHERE player_id = ?", sqlConnection);
	...
	public static void saveIgnoreEvents(boolean ignoreEvents, int userId) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("UPDATE player_settings SET ignore_events = ? WHERE player_id = ?", sqlConnection);
	...
	public static void savePersonalStaff(boolean personalStaff, int userId) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("UPDATE player_settings SET personalstaff = ? WHERE player_id = ?", sqlConnection);
	...
	public static boolean usernameIsAvailable(String username) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("SELECT `id` FROM players WHERE username = ? LIMIT 1", sqlConnection);
			preparedStatement.setString(1, username);
	...
	public static void resetHomeRoom(int roomId) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("UPDATE player_settings SET home_room = 0 WHERE home_room = ?", sqlConnection);
	...
	public static void resetOnlineStatus() {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("UPDATE players SET online = '0' WHERE online = '1'", sqlConnection);
	...
	public static void nullifyAuthTicket(int playerId) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("UPDATE players SET auth_ticket = NULL WHERE id = ?", sqlConnection);
	...
	public static void saveBatch(Map<Integer, PlayerData> playerData) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("UPDATE players SET username = ?, motto = ?, figure = ?, credits = ?, vip_points = ?, gender = ?, favourite_group = ?, activity_points = ?, quest_id = ?, achievement_points = ? WHERE id = ?;", sqlConnection);
	...
	public static void dailyPlayerUpdate(int dailyRespects, int dailyScratches) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("UPDATE player_stats SET daily_respects = ?, daily_scratches = ?;", sqlConnection);
	...
	public static int getUsernameAlreadyExist(String username) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("SELECT COUNT(0) as exist FROM players WHERE username = ?", sqlConnection);
			preparedStatement.setString(1, username);
	...
	public static void updatePlayersUsername(String newName, int playerId) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("UPDATE players SET username = ? WHERE id = ?", sqlConnection);
	...
	public static void updateRoomsUsername(String newName, int playerId) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("UPDATE rooms SET owner = ? WHERE owner_id = ?", sqlConnection);
			preparedStatement.setString(1, newName);
			preparedStatement.setInt(2, playerId);
	...
	public static void saveNavigatorSettings(int navigatorX, int navigatorY, int navigatorHeight, int navigatorWidth, boolean navigatorShowSearches, int playerId) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("UPDATE player_settings SET navigator_x = ?, navigator_y = ?, navigator_height = ?, navigator_width = ?, navigator_show_searches = ? WHERE player_id = ?", sqlConnection);
	...
	public static Set<Integer> getFavouriteRooms(int playerId) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("SELECT `room_id` FROM `player_favourite_rooms` WHERE `player_id` = ?", sqlConnection);
			preparedStatement.setInt(1, playerId);
	...
	public static void deleteFavouriteRoom(int playerId, int roomId) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("DELETE FROM player_favourite_rooms WHERE player_id = ? AND room_id = ?", sqlConnection);
	...
	public static Map<Integer, ISavedSearch> getSavedSearches(int playerId) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("SELECT `id`, `view`, `search_query` FROM player_saved_searches WHERE player_id = ? LIMIT 50;", sqlConnection);
			preparedStatement.setInt(1, playerId);
	...
	public static Map<String, Integer> getViewModes(int playerId) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("SELECT * FROM player_navigator_view_modes WHERE player_id = ? LIMIT 50;", sqlConnection);
			preparedStatement.setInt(1, playerId);
	...
	public static int saveSearch(int playerId, SavedSearch savedSearch) {
		...
		try {
			...
			preparedStatement.setString(3, savedSearch.searchQuery());
	...
	public static void deleteSearch(int searchId) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("DELETE FROM player_saved_searches WHERE id = ?", sqlConnection);
	...
	public static Set<Integer> getEffects(int playerId) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("SELECT `effect_id` FROM `player_effects` WHERE `player_id` = ?;", sqlConnection);
			preparedStatement.setInt(1, playerId);
	...
	public static void saveMentionType(int playerId, MentionType mentionType) {
		...
		try {
			...
			preparedStatement = SQLUtility.prepare("UPDATE player_settings SET mention_type = ? WHERE player_id = ?", sqlConnection);
