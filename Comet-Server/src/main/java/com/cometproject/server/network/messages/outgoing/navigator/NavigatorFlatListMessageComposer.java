...
public class NavigatorFlatListMessageComposer extends MessageComposer {
	...
	public void compose(IComposerDataWrapper msg) {
		...
		msg.writeInt(limit ? (Math.min(activeRooms.size(), 50)) : activeRooms.size());
		
		if (order) {
			try {
				((List<IRoomData>) activeRooms).sort((o1, o2) -> {
					boolean is1Active = RoomManager.getInstance().isActive(o1.getId());
					boolean is2Active = RoomManager.getInstance().isActive(o2.getId());
					
					return ((!is2Active ? 0 : RoomManager.getInstance().get(o2.getId()).getEntities().playerCount()) - (!is1Active ? 0 : RoomManager.getInstance().get(o1.getId()).getEntities().playerCount()));
				});
		...
		for (IRoomData room : activeRooms) {
			if (topRooms.size() < 50 || !limit) topRooms.add(room);
		}
