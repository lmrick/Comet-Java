	...
	public void handle(Session client, MessageEvent msg) {
		...
		if (data == null) {
			if (NetworkManager.getInstance().getSessions().getByPlayerId(userId) != null) {
				data = Objects.requireNonNull(NetworkManager.getInstance().getSessions().getByPlayerId(userId)).getPlayer().getData();
				stats = Objects.requireNonNull(NetworkManager.getInstance().getSessions().getByPlayerId(userId)).getPlayer().getStats();
				groups = Objects.requireNonNull(NetworkManager.getInstance().getSessions().getByPlayerId(userId)).getPlayer().getGroups();
			}
		}
