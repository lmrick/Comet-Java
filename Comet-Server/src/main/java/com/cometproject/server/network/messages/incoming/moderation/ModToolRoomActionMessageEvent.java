...
public class ModToolRoomActionMessageEvent implements Event {
	...
	public void handle(Session client, MessageEvent msg) throws Exception {
		...
		if (isActive) {
			...
			if (lockDoor) room.getData().setAccess(RoomAccessType.DOORBELL);
			
			GameContext.getCurrent().getRoomService().saveRoomData(room.getData());
			
			if (kickAll) {
				room.getEntities().getPlayerEntities().stream().filter(entity -> entity.getPlayer().getPermissions().getRank().roomKickable()).forEachOrdered(PlayerEntity::kick);
			}
