...
public class RedeemVoucherMessageEvent implements Event {
	...
	public void handle(Session client, MessageEvent msg) throws Exception {
		...
		if (client.getPlayer().getVoucherRedeemAttempts() >= 20 && (client.getPlayer().getLastVoucherRedeemAttempt() + 120) > (System.currentTimeMillis() / 1000)) {
			return;
		}
		...
		switch (voucher.type()) {
			case COINS: {
				...
				if (!StringUtils.isNumeric(voucher.data())) {
					failure = true;
					break;
				}
				
				final int coinAmount = Integer.parseInt(voucher.data());
			...
			case DUCKETS: {
				...
				final int ducketAmount = Integer.parseInt(voucher.data());
			...
			case VIP_POINTS: {
				...
				final int vipPointAmount = Integer.parseInt(voucher.data());
			...
			case ROOM_BUNDLE: {
				RoomBundle roomBundle = RoomBundleManager.getInstance().getBundle(voucher.data());
		...
		VoucherDao.claimVoucher(voucher.id(), client.getPlayer().getId());
