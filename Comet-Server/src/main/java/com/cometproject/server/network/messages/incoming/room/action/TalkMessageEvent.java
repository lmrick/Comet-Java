...
public class TalkMessageEvent implements Event {
	...
	public void handle(Session client, MessageEvent msg) {
		...
		if (playerEntity == null || playerEntity.getRoom() == null || playerEntity.getRoom().getEntities() == null) return;
			...
			if (message.startsWith("@")) {
				...
				if (player != null) {
					if (player.getWsChannel() != null) {
						...
						notificationParams.put("message", Locale.getOrDefault("mention.message", "The user %s has mentioned you in a room (%b), click here to go to the room.").replace("%s", client.getPlayer().getData().getUsername()).replace("%b", message));
					...
					client.send(new WhisperMessageComposer(client.getPlayer().getData().getId(), Locale.getOrDefault("mention.success", "You've mention %s successfully").replace("%s", finalName), 34));
				} else {
					client.send(new WhisperMessageComposer(client.getPlayer().getData().getId(), Locale.getOrDefault("mention.notexist", "The user %s does not exist or it's disconnected").replace("%s", finalName), 34));
				}
			}
