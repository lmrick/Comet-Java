...
public class PlacePetMessageEvent implements Event {
	...
	public void handle(Session client, MessageEvent msg) throws Exception {
		...
		if (isOwner || room.getData().isAllowPets()) {
			...
			if ((!atDoor && !tile.getEntities().isEmpty()) || room.getMapping().isValidPosition(position) || room.getModel().getSquareState()[x][y] != RoomTileState.VALID) {
				return;
			}
			...
			room.getItems().getItemsOnSquare(x, y).forEach(floorItem -> floorItem.onEntityStepOn(petEntity));
