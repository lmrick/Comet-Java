...
public class RoomMuteCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
			...
			for (PlayerEntity entity : client.getPlayer().getEntity().getRoom().getEntities().getPlayerEntities()) {
				if (entity.getEntityType() == RoomEntityType.PLAYER) {
					...
					if (entity.getPlayer().getSession().getWsChannel() != null) {
						entity.getPlayer().getSession().sendWs(new MutedMessage(MutedMessage.MuteType.ROOM_MUTE, false, null));
					} else {
						entity.getPlayer().getSession().send(new WhisperMessageComposer(entity.getId(), Locale.getOrDefault("command.room.unmute", "You are now able to chat again :-)")));
					}
				}
			}
		...
		this.logDesc = "%s has muted room '%b'".replace("%s", client.getPlayer().getData().getUsername()).replace("%b", client.getPlayer().getEntity().getRoom().getData().getName());
