...
public class ColourCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
		if (params.length != 1) {
			...
			sendNotification(Locale.getOrDefault("command.colour.reset", "Your chat has been returned to normal!"), client);
			return;
		}
		...
		if (params[0].toLowerCase().equals(Locale.getOrDefault("command.colour.reset.word", "normal"))) {
			...
			sendNotification(Locale.getOrDefault("command.colour.reset", "Your chat colour has been returned to normal!"), client);
			return;
		}
		...
		try {
			...
			sendNotification(Locale.getOrDefault("command.colour.done", "Your chat messages are now %colour%").replace("%colour%", colourString), client);
		} catch (Exception e) {
			sendNotification(Locale.getOrDefault("command.colour.invalid", "Invalid colour, available colours: %colours%").replace("%colours%", ChatMessageColour.getAllAvailable()), client);
		}
	}
