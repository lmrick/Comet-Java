...
public class ListenCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
		if (params.length != 1) {
			sendNotification(Locale.get("command.listen.notfound"), client);
			return;
		}
		...
		if (session == null || session.getPlayer() == null || session.getPlayer().getData().getRank() > client.getPlayer().getData().getRank()) {
			sendNotification(Locale.get("command.listen.notfound"), client);
			return;
		}
		...
		if (session.getPlayer().getListeningPlayers().contains(client.getPlayer().getId())) {
			...
			sendNotification(Locale.get("command.listen.listening").replace("%username%", session.getPlayer().getData().getUsername()), client);
		}
	}
