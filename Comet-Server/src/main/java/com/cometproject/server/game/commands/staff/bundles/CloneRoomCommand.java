...
public class CloneRoomCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
		...
		try {
			...
			bundle.getRoomBundleData().forEach(roomBundleItem -> {
			...
			this.logDesc = "Staff %s cloned room '%b'".replace("%s", client.getPlayer().getData().getUsername()).replace("%b", client.getPlayer().getEntity().getRoom().getData().getName());
