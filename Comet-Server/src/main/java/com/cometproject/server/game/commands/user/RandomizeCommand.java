...
public class RandomizeCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
		if (params.length != 2) {
			sendNotification(Locale.getOrDefault("command.randomize.none", "Between which 2 numbers do you want to randomize?"), client);
			return;
		}
		...
		try {
			...
			if (min > 1000 || max > 1000) {
				sendNotification(Locale.getOrDefault("command.randomize.toohigh", "Your min or max number is too high. Choose one between 0 and 1000!"), client);
				return;
			} else if (min < 0 || max < 0) {
				sendNotification(Locale.getOrDefault("command.randomize.negative", "You can only use positive numbers!"), client);
				return;
			}
			...
			sendNotification(Locale.get("command.randomize.invalidnumbers"), client);
		}
	}
