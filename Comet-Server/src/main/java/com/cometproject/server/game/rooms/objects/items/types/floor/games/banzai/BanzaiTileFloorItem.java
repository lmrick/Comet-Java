...
public class BanzaiTileFloorItem extends RoomItemFloor {
	...
	public void onEntityPostStepOn(RoomEntity entity) {
		final RoomGame gameInstance = this.getRoom().getGame().getInstance();
		if (gameInstance == null || !gameInstance.isActive() || !(entity instanceof PlayerEntity playerEntity)) {
			return;
		}
		...
		if (this.points == 3) {
			...
			if (rectangle != null) {
				this.getRoom().getItems().getByClass(BanzaiTileFloorItem.class).stream().filter(floorItem -> floorItem.getPoints() != 3).forEachOrdered(floorItem -> {
					final boolean[] borderCheck = new boolean[4];
					rectangle.forEach(rectangleItem -> {
	...
	private static List<BanzaiTileFloorItem> buildBanzaiRectangle(final BanzaiTileFloorItem triggerItem, final int x, final int y, final int goX, final int goY, final int currentDirection, final int turns, final GameTeam team) {
		...
		for (int i = 0; i < 4; ++i) {
			...
			if (room.getMapping().getTile(nextX, nextY) != null) {
				...
				if (obj instanceof BanzaiTileFloorItem item) {
					
					if (item.getTeam() == team && item.getPoints() == 3) {
						List<BanzaiTileFloorItem> foundPatches = null;
						if (currentDirection != i && currentDirection != -1) {
							if (turns > 0) {
								foundPatches = buildBanzaiRectangle(triggerItem, nextX, nextY, (nextXStep == 0) ? (goX * -1) : (nextXStep * -1), (nextYStep == 0) ? (goY * -1) : (nextYStep * -1), i, (turns - 1), team);
							}
						} else {
							foundPatches = buildBanzaiRectangle(triggerItem, nextX, nextY, (nextXStep == 0) ? goX : (nextXStep * -1), (nextYStep == 0) ? goY : (nextYStep * -1), i, turns, team);
						}
	...
	public void updateTileData() {
		this.getItemData().setData(this.points != 0 ? ((this.points + (gameTeam.getTeamId() * 3) - 1) + "") : "1");
		this.sendUpdate();
	}
