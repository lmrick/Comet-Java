...
public class MentionSettingsCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
		...
		if (mentionType.equals(mentionAll)) {
			...
			sendNotification(Locale.getOrDefault("mention.all.set", "Mentions are now open to everyone!"), client);
		} else if (mentionType.equals(mentionFriends)) {
			playerSettings.setMentionType(MentionType.FRIENDS);
			sendNotification(Locale.getOrDefault("mention.friends.set", "Mentions are now open to friends only!"), client);
		} else {
			playerSettings.setMentionType(MentionType.NONE);
			sendNotification(Locale.getOrDefault("mention.friends.set", "Mentions are now disabled"), client);
		}
