...
public class PointsCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
		...
		session.send(new AdvancedAlertMessageComposer(Locale.get("command.points.successtitle"), Locale.get("command.points.successmessage").replace("%amount%", String.valueOf(points)).replace("%type%", Locale.getOrDefault("command.points.currency." + currency, currency))));
		
		session.send(session.getPlayer().composeCurrenciesBalance());
		
		this.logDesc = "El staff %s ha dado %c %l al usuario '%u'".replace("%l", Locale.get("command.points.currency." + currency)).replace("%c", params[1]).replace("%s", client.getPlayer().getData().getUsername()).replace("%u", username);
	}
