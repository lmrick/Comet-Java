...
public abstract class MassCurrencyCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
		if (params.length < 1 || params[0].isEmpty() || !StringUtils.isNumeric(params[0])) return;
		...
		for (ISession session : NetworkManager.getInstance().getSessions().getSessions().values()) {
			try {
				...
				if (this instanceof MassCoinsCommand) {
					...
					this.logDesc = "El staff %s ha dado a todo el hotel %n duckets".replace("%n", Integer.toString(amount));
					
				} else if (this instanceof MassPointsCommand) {
					session.getPlayer().getData().increaseVipPoints(amount);
					currencyType = "vip.points";
					this.logDesc = "El staff %s ha dado a todo el hotel %n diamantes".replace("%n", Integer.toString(amount));
					
				} else if (this instanceof MassSeasonalCommand) {
					session.getPlayer().getData().increaseSeasonalPoints(amount);
					currencyType = "seasonal";
					this.logDesc = "El staff %s ha dado a todo el hotel %n boolares".replace("%n", Integer.toString(amount));
				}
				
				if (!currencyType.equals("coins")) {
					session.send(session.getPlayer().composeCurrenciesBalance());
					this.logDesc = "El staff %s ha dado a todo el hotel %n créditos".replace("%n", Integer.toString(amount));
				}
				
				session.send(new AdvancedAlertMessageComposer(Locale.get("command.points.successtitle"), Locale.get("command.points.successmessage").replace("%amount%", String.valueOf(amount)).replace("%type%", Locale.get(currencyType + ".name"))));
