...
public class RoomNotificationCommand extends NotificationCommand {
	...
	protected void globalNotification(String image, String message, Session client) {
		...
		this.logDesc = "El staff %s ha hecho roomnotification en la sala '%b' con parámetro %p".replace("%s", client.getPlayer().getData().getUsername()).replace("%b", client.getPlayer().getEntity().getRoom().getData().getName()).replace("%p", message);
	}
