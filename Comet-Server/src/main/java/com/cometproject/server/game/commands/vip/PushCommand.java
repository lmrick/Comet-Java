...
public class PushCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
		...
		if (user == client) {
			sendNotification(Locale.get("command.push.playerhimself"), client);
			return;
		}
	...
	private void push(RoomEntity entity, Session client) {
		...
		if (!((Math.abs((posX - playerX)) >= 2) || (Math.abs(posY - playerY) >= 2))) {
			...
			client.getPlayer().getEntity().getRoom().getEntities().broadcastMessage(new TalkMessageComposer(client.getPlayer().getEntity().getId(), Locale.get("command.push.message").replace("%playername%", entity.getUsername()), ChatEmotion.NONE, 0));
		} else {
