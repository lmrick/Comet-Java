...
public class MessengerComponent extends PlayerComponent implements IPlayerMessenger {
	...
	public void broadcast(IMessageComposer msg) {
		this.getFriends().values().stream().filter(friend -> friend.isOnline() && friend.getUserId() != this.getPlayer().getId()).map(friend -> NetworkManager.getInstance().getSessions().getByPlayerId(friend.getUserId())).filter(session -> session != null && session.getPlayer().getMessenger().isInitialized()).forEachOrdered(session -> session.send(msg));
	}
	...
	public void sendStatus(boolean online, boolean inRoom) {
		...
		this.getFriends().values().stream().filter(friend -> friend.isOnline() && friend.getUserId() != this.getPlayer().getId()).map(friend -> NetworkManager.getInstance().getSessions().getByPlayerId(friend.getUserId())).filter(session -> session != null && session.getPlayer().getMessenger().isInitialized()).forEachOrdered(session -> session.send(new UpdateFriendStateMessageComposer(this.getPlayer().getData(), online, inRoom, session.getPlayer().getRelationships().get(this.getPlayer().getId()))));
