...
public class RoomBadgeCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
		...
		this.logDesc = "El staff %s ha hecho roombadge con código '%c' en la sala %r".replace("%s", client.getPlayer().getData().getUsername()).replace("%c", badge).replace("%r", client.getPlayer().getEntity().getRoom().getData().getName());
	}
