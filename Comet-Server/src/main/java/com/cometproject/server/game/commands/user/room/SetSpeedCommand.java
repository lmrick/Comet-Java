	...
	public void execute(Session client, String[] params) {
		...
		if (client.getPlayer().getEntity() != null && client.getPlayer().getEntity().getRoom() != null) {
			if (!client.getPlayer().getEntity().getRoom().getRights().hasRights(client.getPlayer().getId()) && !client.getPlayer().getPermissions().getRank().roomFullControl()) {
				sendNotification(Locale.getOrDefault("command.need.rights", "You need rights to use this command!"), client);
				return;
			}
			...
			try {
				...
				sendNotification(Locale.get("command.setspeed.set").replace("%s", speed + ""), client);
			} catch (Exception e) {
				sendNotification(Locale.getOrDefault("command.setspeed.invalid", "Please, use numbers only!"), client);
			}
		}
	}
