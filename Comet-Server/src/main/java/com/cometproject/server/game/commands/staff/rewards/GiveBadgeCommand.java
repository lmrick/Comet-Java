...
public class GiveBadgeCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
		...
		if (session != null) {
			...
			sendNotification(Locale.get("command.givebadge.success").replace("%username%", username).replace("%badge%", badge), client);
		} else {
			int playerId = PlayerDao.getIdByUsername(username);
			
			if (playerId == 0) {
				sendNotification(Locale.get("command.givebadge.fail").replace("%username%", username).replace("%badge%", badge), client);
			} else {
		...
		this.logDesc = "El Staff -c le ha dado la placa -d a -e".replace("-c", client.getPlayer().getData().getUsername()).replace("-d", badge).replace("-e", username);
	}
