	...
	public void execute(Session client, String[] params) {
		...
		switch (commandName) {
			case "create" -> {
				...
				boolean updateCatalog = RoomBundleManager.getInstance().getBundle(alias) != null;
				...
				this.logDesc = "El staff %s ha hecho bundle en la sala '%b' con el nombre %n".replace("%s", client.getPlayer().getData().getUsername()).replace("%b", client.getPlayer().getEntity().getRoom().getData().getName()).replace("%n", alias);
