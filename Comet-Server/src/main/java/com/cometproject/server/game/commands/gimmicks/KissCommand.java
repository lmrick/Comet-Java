...
public class KissCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
		if (params.length != 1) {
			sendNotification(Locale.getOrDefault("command.kiss.none", "Who do you want to kiss?"), client);
			return;
		}
		...
		if (kissedSession.getPlayer().getData().getUsername().equals(client.getPlayer().getData().getUsername())) {
			sendNotification(Locale.getOrDefault("command.kiss.himself", "You can't kiss yourself!"), client);
			return;
		}
