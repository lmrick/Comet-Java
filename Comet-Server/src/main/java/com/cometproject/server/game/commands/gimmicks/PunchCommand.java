...
public class PunchCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
		if (params.length != 1) {
			sendNotification(Locale.getOrDefault("command.punch.none", "Who do you want to punch?"), client);
			return;
		}
		...
		if (Objects.equals(punchedSession.getPlayer().getData().getUsername(), client.getPlayer().getData().getUsername())) {
			sendNotification(Locale.getOrDefault("command.punch.himself", "You can't punch yourself!"), client);
			return;
		}
