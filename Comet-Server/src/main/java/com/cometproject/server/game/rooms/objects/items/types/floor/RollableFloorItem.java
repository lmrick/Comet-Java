...
public abstract class RollableFloorItem extends RoomItemFloor {
	...
	public void onTickComplete() {
		...
		if (isStart) {
			...
			Position newPosition = this.isValidRoll(nextPosition) ? nextPosition : this.getNextPosition();
				...
				switch (rotation) {
					...
					case Position.SOUTH_EAST -> {
						...
						if (!this.isValidRoll(position.squareInFront(rotation))) {
							...
							this.isValidRoll(position.squareInFront(rotation));
						}
					}
	...
	private void moveTo(Position pos, int rotation) {
		...
		this.getRoom().getItems().getItemsOnSquare(pos.getX(), pos.getY()).forEach(floorItem -> floorItem.onItemAddedToStack(this, this.kickerEntity));
