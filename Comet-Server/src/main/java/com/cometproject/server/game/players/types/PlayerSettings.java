...
public class PlayerSettings implements IPlayerSettings {
	...
	public PlayerSettings(ResultSet data, boolean isLogin, Player player) throws SQLException {
		if (isLogin) {
			...
			volumes = volumeData != null && volumeData.startsWith("{") ? JsonUtil.getInstance().fromJson(volumeData, VolumeData.class) : new VolumeData(100, 100, 100);
	...
	public JsonObject toJson() {
		...
		wardrobe.stream().map(IWardrobeItem::toJson).forEach(wardrobeArray::add);
