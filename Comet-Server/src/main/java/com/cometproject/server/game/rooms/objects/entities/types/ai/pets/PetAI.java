...
public class PetAI extends AbstractBotAI {
	...
	public static final List<Integer> levelBoundaries = Arrays.asList(0, 100, 200, 400, 600, 900, 1300, 1800, 2400, 3200, 4300, 5700, 7600, 10100, 13300, 17500, 23000, 30200, 39600, 51900);
	private static final PetAction[] possibleActions = { PetAction.LAY, PetAction.SIT, PetAction.TALK, PetAction.PLAY };
	...
	public void onTickComplete() {
		...
		if (this.getPetEntity().getData().getHunger() >= 65) {
			...
			switch (petAction) {
				case TALK -> this.say(this.getMessage(PetMessageType.GENERIC));
				case LAY -> this.lay();
				case SIT -> this.sit();
				case PLAY -> this.play();
			}
	...
	public void tryNest() {
		...
		if (petNest != null) {
			if (petNest.getTile().getEntities().isEmpty()) {
	...
	public void tryEat() {
		...
		if (petFood != null) {
			if (petFood.getTile().getEntities().isEmpty()) {
	...
	private boolean canGesture() {
		switch (this.getPetEntity().getData().getTypeId()) {
			case PetType.DRAGON, PetType.HORSE, PetType.SPIDER, PetType.FROG, PetType.CHICKEN, PetType.TURTLE -> {
				return true;
			}
		}
	...
	public void increaseExperience(int amount) {
		...
		for (Integer levelBoundary : levelBoundaries) {
			...
			if (this.getPetEntity().getData().getLevel() < level && this.getPetEntity().getData().getExperience() >= levelBoundary) {
	...
	private String getMessage(PetMessageType type) {
		...
		if (message.contains("%ownerName%")) {
			if (this.ownerName.isEmpty()) {
				IPlayerAvatar playerAvatar = PlayerManager.getInstance().getAvatarByPlayerId(this.getPetEntity().getData().getOwnerId(), IPlayerAvatar.USERNAME_FIGURE);
	...
	private BreedingBoxFloorItem findBreedingBox() {
		...
		return availableBoxes == null ? null : availableBoxes.stream().filter(box -> box.getTile().getEntities().size() <= 1).findFirst().orElse(null);
	}
