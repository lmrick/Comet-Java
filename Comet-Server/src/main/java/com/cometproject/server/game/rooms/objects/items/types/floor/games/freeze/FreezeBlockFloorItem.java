	...
	public boolean onInteract(RoomEntity entity, int requestData, boolean isWiredTrigger) {
		if (this.destroyed) {
			...
			if (roomTile.getItems().size() > 1) {
				return roomTile.getItems().stream().filter(FreezeTileFloorItem.class::isInstance).findFirst().filter(floorItem -> floorItem.onInteract(entity, requestData, isWiredTrigger)).isPresent();
			}
		}
