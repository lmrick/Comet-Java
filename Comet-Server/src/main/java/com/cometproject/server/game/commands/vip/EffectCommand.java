...
public class EffectCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
		if (params.length != 1) {
			sendNotification(Locale.getOrDefault("command.enable.none", "To get a effect type :effect %number%"), client);
			return;
		}
		...
		try {
			...
			sendNotification(Locale.get("command.enable.invalidid"), client);
		}
	}
