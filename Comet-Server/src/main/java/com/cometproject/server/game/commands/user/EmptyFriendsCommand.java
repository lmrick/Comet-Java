...
public class EmptyFriendsCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
		...
		for (Integer playerId : client.getPlayer().getMessenger().getFriends().keySet()) {
			...
			if (removedUser == null) continue;
			
			
			if (client.getPlayer().getRelationships().getRelationships().containsKey(playerId)) {
				client.send(new UpdateFriendStateMessageComposer(removedUser.getPlayer().getData(), removedUser.getPlayer().isOnline(), removedUser.getPlayer().getEntity() != null, removedUser.getPlayer().getRelationships().get(removedUser.getPlayer().getData().getId())));
				
				if (removedUser.getPlayer().getRelationships().getRelationships().containsKey(client.getPlayer().getData().getId())) {
					removedUser.send(new UpdateFriendStateMessageComposer(client.getPlayer().getData(), client.getPlayer().isOnline(), client.getPlayer().getEntity() != null, client.getPlayer().getRelationships().get(client.getPlayer().getData().getId())));
				}
			}
