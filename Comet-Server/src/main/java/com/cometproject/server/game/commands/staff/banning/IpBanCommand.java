...
public class IpBanCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
		...
		if (BanManager.getInstance().hasBan(ipAddress, BanType.IP)) {
			sendNotification("IP: " + ipAddress + " is already banned.", client);
			return;
		}
		...
		sendNotification("User has been IP banned (IP: " + ipAddress + ")", client);
		...
		this.logDesc = "El Staff -c le ha dado IPBan a -d".replace("-c", client.getPlayer().getData().getUsername()).replace("-d", username);
	}
