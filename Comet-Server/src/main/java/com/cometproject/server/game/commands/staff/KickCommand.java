...
public class KickCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
		if (params.length < 1) {
			sendNotification(Locale.getOrDefault("command.kick.none", "Who do you want to kick?"), client);
			return;
		}
		...
		if (!user.getPlayer().getPermissions().getRank().roomKickable()) {
			sendNotification(Locale.getOrDefault("command.kick.unkickable", "You can't kick this player!"), client);
			return;
		}
		...
		this.logDesc = "El Staff -c ha kickeado al usuario -d".replace("-c", client.getPlayer().getData().getUsername()).replace("-d", user.getPlayer().getData().getUsername());
	}
