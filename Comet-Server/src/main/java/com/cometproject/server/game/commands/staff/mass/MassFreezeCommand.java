...
public class MassFreezeCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
			...
			if (roomEntity != client.getPlayer().getEntity() && !(roomEntity instanceof PetEntity)) {
				...
				if (!roomEntity.canWalk()) {
					if (roomEntity instanceof PlayerEntity) {
						sendNotification(Locale.getOrDefault("command.massfreeze.frozen", "You've been frozen!"), ((PlayerEntity) roomEntity).getPlayer().getSession());
					}
					
					roomEntity.applyEffect(new PlayerEffect(12));
				} else {
					if (roomEntity instanceof PlayerEntity) {
						sendNotification(Locale.getOrDefault("command.massfreeze.unfrozen", "You've been frozen!"), ((PlayerEntity) roomEntity).getPlayer().getSession());
					}
		...
		this.logDesc = "%s executed massfreeze".replace("%s", client.getPlayer().getData().getUsername());
	}
