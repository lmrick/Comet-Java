...
public class UnmuteCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
		if (params.length != 1) {
			sendNotification(Locale.getOrDefault("command.unmute.none", "Who do you want to unmute?"), client);
			return;
		}
		...
		this.logDesc = "%s unmuted '%u'".replace("%s", client.getPlayer().getData().getUsername()).replace("%u", params[0]);
	}
