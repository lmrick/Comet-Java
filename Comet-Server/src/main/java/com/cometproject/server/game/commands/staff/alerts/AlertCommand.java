...
public class AlertCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
		if (params.length < 2) {
			sendNotification(Locale.getOrDefault("command.alert.none", "Who do you want to send a alert?"), client);
			return;
		}
		...
		this.logDesc = "El Staff -c le ha mandado una alerta a -d<. [-e]".replace("-c", client.getPlayer().getData().getUsername()).replace("-d", user.getPlayer().getData().getUsername()).replace("-e", this.merge(params));
	}
