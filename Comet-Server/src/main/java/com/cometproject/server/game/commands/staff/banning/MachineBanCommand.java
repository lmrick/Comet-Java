...
public class MachineBanCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
		...
		if (user == client || !user.getPlayer().getPermissions().getRank().bannable() || user.getPlayer().getPermissions().getRank().getId() >= client.getPlayer().getPermissions().getRank().getId()) {
			sendNotification(Locale.getOrDefault("command.user.notbannable", "You're not able to ban this user!"), client);
			return;
		}
		...
		if (BanManager.getInstance().hasBan(uniqueId, BanType.MACHINE)) {
			sendNotification("Machine ID: " + uniqueId + " is already banned.", client);
			return;
		}
		...
		sendNotification("User has been machine ID banned (" + uniqueId + ")", client);
		
		user.disconnect("banned");
		
		this.logDesc = "El staff %s ha hecho machineban a '%u'".replace("%s", client.getPlayer().getData().getUsername()).replace("%u", username);
	}
