...
public class Poll {
	...
	public void onPlayerFinishedPoll(Player player) {
		...
		if (this.rewardCredits != 0) {
			...
			player.getSession().send(new AlertMessageComposer(Locale.getOrDefault("wired.reward.coins", "You received %s coin(s)!").replace("%s", this.rewardCredits + "")));
		...
		if (this.rewardActivityPoints != 0) {
			...
			player.getSession().send(new AlertMessageComposer(Locale.getOrDefault("wired.reward.duckets", "You received %s ducket(s)!").replace("%s", this.rewardActivityPoints + "")));
		...
		if (this.rewardVipPoints != 0) {
			...
			player.getSession().send(new AlertMessageComposer(Locale.getOrDefault("wired.reward.diamonds", "You received %s diamonds(s)!").replace("%s", this.rewardVipPoints + "")));
			save = true;
		}
