...
public class MassHandItemCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
		if (params.length != 1) {
			sendNotification(Locale.getOrDefault("command.masshanditem.none", "To give everyone in the room an handitem type :masshanditem %number%"), client);
			return;
		}
		...
		try {
			...
			sendNotification(Locale.get("command.masshanditem.invalidid"), client);
		}
		
		this.logDesc = "El staff %s ha hecho <b>massHandItem</b> en la sala '%b'".replace("%s", client.getPlayer().getData().getUsername()).replace("%b", client.getPlayer().getEntity().getRoom().getData().getName());
	}
