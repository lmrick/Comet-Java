...
public class DeleteGroupCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
		...
		if (room.getGroup() != null) {
			...
			for (Integer groupMemberId : group.getMembers().getAll().keySet()) {
				...
				if (groupMemberId != group.getData().getOwnerId()) {
					floorItemsOwnedByPlayer = room.getItems().getFloorItems().values().stream().filter(floorItem -> floorItem.getItemData().getOwnerId() == groupMemberId).collect(Collectors.toList());
				...
				if (groupMemberSession != null && groupMemberSession.getPlayer() != null) {
					...
					floorItemsOwnedByPlayer.forEach(roomItem -> {
					...
					floorItemsOwnedByPlayer.forEach(roomItem -> StorageContext.getCurrentContext().getRoomItemRepository().removeItemFromRoom(roomItem.getId(), groupMemberId, roomItem.getItemData().getData()));
				}
		...
		this.logDesc = "El Staff %s ha eliminado el grupo %b cuya sala es %d".replace("%s", client.getPlayer().getData().getUsername()).replace("%b", this.groupName).replace("%d", room.getData().getName());
	}
