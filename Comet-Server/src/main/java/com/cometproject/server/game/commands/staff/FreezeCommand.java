...
public class FreezeCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
		if (params.length != 1) {
			sendNotification(Locale.getOrDefault("command.freeze.none", "Who do you want to freeze?"), client);
			return;
		}
		...
		this.logDesc = "El staff %s ha hecho freeze en la sala '%b'".replace("%s", client.getPlayer().getData().getUsername()).replace("%b", client.getPlayer().getEntity().getRoom().getData().getName());
	}
