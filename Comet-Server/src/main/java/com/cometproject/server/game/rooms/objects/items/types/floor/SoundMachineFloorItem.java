...
public class SoundMachineFloorItem extends RoomItemFloor implements Stateable {
	...
	public void onTickComplete() {
		...
		if (this.isPlaying) {
			...
			this.getRoom().getEntities().getPlayerEntities().stream().filter(entity -> !entity.hasAttribute("traxSent") && (System.currentTimeMillis() - entity.getJoinTime() >= 1100)).forEachOrdered(entity -> {
			...
			if (this.currentPlayingIndex != -1) {
				...
				if (songItemData != null) {
					...
					if (musicData != null) {
						if (this.timePlaying() >= (musicData.lengthSeconds() + 1.0)) {
							this.playNextSong();
						}
					}
				}
	...
	public void broadcastSong() {
		...
		this.getRoom().getEntities().getPlayerEntities().forEach(entity -> {
	...
	public int songTimeSync() {
		...
		if (songItemData != null) {
			...
			if (musicData != null) {
				if (this.timePlaying() >= musicData.lengthSeconds()) return musicData.lengthSeconds();
			}
		}
