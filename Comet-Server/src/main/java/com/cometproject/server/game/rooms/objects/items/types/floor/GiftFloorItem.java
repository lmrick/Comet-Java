...
public class GiftFloorItem extends RoomItemFloor {
	...
	public GiftFloorItem(RoomItemData roomItemData, Room room) throws Exception {
		...
		playerAvatar = this.giftData.showUsername() && giftData.getSenderId() != 0 ? PlayerManager.getInstance().getAvatarByPlayerId(giftData.getSenderId(), IPlayerAvatar.USERNAME_FIGURE) : null;
	}
	...
	private GiftData loadGiftData(String extraData) throws Exception {
		...
		if (extraData.startsWith(LegacyGiftData.EXTRA_DATA_HEADER)) {
			...
			GiftData newGiftData = legacyGiftData.upgrade(catalogItem.getItems().getFirst().itemId());
