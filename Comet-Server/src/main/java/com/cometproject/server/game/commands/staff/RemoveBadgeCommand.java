...
public class RemoveBadgeCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
		...
		if (session != null) {
			...
			sendNotification(Locale.get("command.removebadge.success").replace("%username%", username).replace("%badge%", badge), client);
		} else {
			sendNotification(Locale.getOrDefault("command.removebadge.userisoff", "This user is not online, you can only remove badges from online users"), client);
		}
		
		this.logDesc = "El Staff -c le ha quitado la placa -d a -e".replace("-c", client.getPlayer().getData().getUsername()).replace("-d", badge).replace("-e", username);
	}
