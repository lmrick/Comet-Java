...
public class FollowCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
		...
		if (leader == client) {
			sendNotification(Locale.getOrDefault("command.follow.playerhimself", "You can't follow yourself!"), client);
			return;
		}
		...
		if (leader != null && leader.getPlayer() != null && leader.getPlayer().getEntity() != null) {
			if (!leader.getPlayer().getSettings().getAllowFollow() && !client.getPlayer().getPermissions().getRank().modTool()) {
				sendNotification(Locale.getOrDefault("command.follow.disabled", "This user has follow disabled."), client);
				return;
			}
			...
			sendNotification(leader == null || leader.getPlayer() == null ? Locale.get("command.follow.online") : Locale.get("command.follow.room"), client);
		}
	}
