...
import com.cometproject.server.game.commands.staff.mass.MassHandItemCommand;
import com.cometproject.server.game.commands.staff.mass.MassTeleportCommand;
...
public class CommandManager implements Initializable {
	...
	public static CommandManager getInstance() {
		if (commandManagerInstance == null) commandManagerInstance = new CommandManager();
	...
	private void loadUserCommands() {
		...
		this.addCommand(Locale.get("command.state.name"), new StateCommand());
	...
	public boolean parse(String message, Session client) {
		...
		if (client.getPlayer().getPermissions().hasCommand(commandName) || commandName.isEmpty()) {
			if (client.getPlayer().getEntity().getRoom().getData().getDisabledCommands().contains(executor)) {
				ChatCommand.sendNotification(Locale.get("command.disabled"), client);
				return true;
			}
			...
			if (PermissionsManager.getInstance().getCommands().containsKey(commandName) && PermissionsManager.getInstance().getCommands().get(commandName).vipOnly() && !client.getPlayer().getData().isVip()) {
				ChatCommand.sendNotification(Locale.get("command.vip"), client);
			}
	...
	private String[] getParams(String[] splitStr) {
		...
		IntStream.range(0, splitStr.length).filter(i -> i != 0).forEachOrdered(i -> a[i - 1] = splitStr[i]);
		return a;
	}
	
	private ChatCommand get(final String executor) {
		if (this.commands.containsKey(executor)) return this.commands.get(executor);
	...
	private void addCommand(String executor, ChatCommand command) {
		...
		keyList.forEach(key -> this.commands.put(":" + key, command));
	}
