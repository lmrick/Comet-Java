...
public class RollCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
		...
		this.logDesc = "Staff %s rigged dice in room '%b' to roll: %r".replace("%s", client.getPlayer().getData().getUsername()).replace("%b", client.getPlayer().getEntity().getRoom().getData().getName()).replace("%r", Integer.toString(number));
	}
