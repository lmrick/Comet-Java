...
public class MassTeleportCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
		...
		this.logDesc = "El staff %s ha hecho massteleport a la sala %b".replace("%s", client.getPlayer().getData().getUsername()).replace("%b", client.getPlayer().getEntity().getRoom().getData().getName());
	}
