...
public class HotelAlertLinkCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
		if (params.length < 2) {
			sendNotification(Locale.getOrDefault("command.hotelalertlink.args", "This command requires at least 2 arguments!"), client);
		}
		...
		this.logDesc = "El staff %s ha mandado una alerta con el enlace '%e' y el mensaje '%m'".replace("%s", client.getPlayer().getData().getUsername()).replace("%e", link).replace("%m", this.merge(params, 1));
	}
