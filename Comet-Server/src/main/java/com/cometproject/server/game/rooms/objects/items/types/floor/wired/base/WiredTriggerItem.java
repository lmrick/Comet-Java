...
public abstract class WiredTriggerItem extends WiredFloorItem {
	...
	public static <T extends RoomItemFloor> List<T> getTriggers(Room room, Class<T> clazz) {
		...
		room.getItems().getByClass(clazz).stream()
						.filter(floorItem -> triggers.size() <= CometSettings.wiredMaxTriggers)
						.forEachOrdered(triggers::add);
	...
	public boolean evaluate(RoomEntity entity, Object data) {
		try {
			...
			wiredConditions.forEach(conditionItem -> {
				conditionItem.flash();
			...
			for (var conditionState : completedConditions.entrySet()) {
				...
				if (conditionState.getKey() instanceof WiredConditionTriggererOnFurni && !(conditionState.getKey() instanceof WiredNegativeConditionTriggererOnFurni)) {
					if (conditionState.getValue().get()) {
			...
			if (canExecute && !wiredActions.isEmpty()) {
				...
				if (unseenEffectItem != null) {
					for (WiredActionItem actionItem : wiredActions) {
						if (!unseenEffectItem.getSeenEffects().contains(actionItem.getId())) {
							...
							if (this.executeEffect(actionItem, entity, data)) wasSuccess = true;
							break;
						}
					}
	...
	public List<WiredActionItem> getIncompatibleActions() {
		...
		if (!this.suppliesPlayer()) {
			...
			incompatibleActions = this.getItemsOnStack().stream()
							.filter(WiredActionItem.class::isInstance)
							.filter(floorItem -> ((WiredActionItem) floorItem).requiresPlayer())
