...
public class RoomAlertCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
		...
		this.logDesc = "-c executed roomalert in -d. [-e]".replace("-c", client.getPlayer().getData().getUsername()).replace("-d", client.getPlayer().getEntity().getRoom().getData().getName()).replace("-e", this.merge(params));
	}
