...
public class SetMaxCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
		if (params.length != 1) {
			sendNotification(Locale.get("command.setmax.invalid"), client);
			return;
		}
		...
		if (hasRights || isStaff) {
			try {
				...
				if ((maxPlayers > CometSettings.roomMaxPlayers && !isStaff) || maxPlayers < 1) {
					sendNotification(Locale.get("command.setmax.toomany").replace("%i", CometSettings.roomMaxPlayers + ""), client);
					return;
				}
				...
				sendNotification(Locale.get("command.setmax.done").replace("%i", maxPlayers + ""), client);
