...
public class UnfreezeCommand extends ChatCommand {
	...
	public void execute(Session client, String[] params) {
		...
		if (params.length != 1) {
			sendNotification(Locale.getOrDefault("command.unfreeze.none", "Who do you want to unfreeze?"), client);
			return;
		}
		...
		this.logDesc = "El staff %s ha hecho unfreeze en la sala '%b'".replace("%s", client.getPlayer().getData().getUsername()).replace("%b", client.getPlayer().getEntity().getRoom().getData().getName());
	}
