...
public class EventRewardCommand extends NotificationCommand {
	...
	public void execute(Session client, String[] params) {
		...
		if (received.has() && received.get()) {
			...
			if (session != null) {
				...
				if (Locale.getOrDefault("notification.eventwin.enabled", "true").equals("true")) {
					globalNotification("generic", Locale.getOrDefault("notification.eventwin", "%username% just won an event!").replace("%username%", session.getPlayer().getData().getUsername()), session);
				}
		...
		this.logDesc = "El staff %s ha hecho eventreward al usuario '%b'".replace("%s", client.getPlayer().getData().getUsername()).replace("%b", username);
	}
