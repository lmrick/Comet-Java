...
public class PickAllCommand extends ChatCommand {
	...
	public void execute(Session client, String[] message) {
		...
		if (room == null || !room.getData().getOwner().equals(client.getPlayer().getData().getUsername())) {
			sendNotification(Locale.getOrDefault("command.need.rights", "You have no rights to use this command in this room."), client);
			return;
		}
		...
		this.logDesc = "El staff %s ha hecho pickall en la sala '%b', cuyo dueño es %c".replace("%s", client.getPlayer().getData().getUsername()).replace("%b", room.getData().getName()).replace("%c", room.getData().getOwner());
	}
