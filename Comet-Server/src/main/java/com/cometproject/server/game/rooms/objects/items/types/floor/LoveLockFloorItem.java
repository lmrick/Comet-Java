...
public class LoveLockFloorItem extends RoomItemFloor {
	...
	public void composeItemData(IComposer msg) {
		...
		Arrays.stream(loveLockData).forEachOrdered(msg::writeString);
	}
	...
	public boolean onInteract(RoomEntity entity, int requestData, boolean isWiredTrigger) {
		...
		Position rightPosition = switch (this.getRotation()) {
			case 2 -> {
				leftPosition = new Position(this.getPosition().getX(), this.getPosition().getY() - 1);
				yield new Position(this.getPosition().getX(), this.getPosition().getY() + 1);
			}
			case 4 -> {
				leftPosition = new Position(this.getPosition().getX() + 1, this.getPosition().getY());
				yield new Position(this.getPosition().getX() - 1, this.getPosition().getY());
			}
			default -> null;
		};
