...
public class LegacyPurchaseHandler implements ICatalogPurchaseHandler {
		...
		try {
			...
			for (ICatalogBundledItem bundledItem : item.getItems()) {
				...
				if (def.getItemType() == ItemType.EFFECT) {
					...
					extraData += client.getPlayer().getData().getUsername() + Character.toChars(9)[0] + DateTime.now().getDayOfMonth() + "-" + DateTime.now().getMonthOfYear() + "-" + DateTime.now().getYear() + Character.toChars(9)[0] + data;
					...
					if (!client.getPlayer().getGroups().contains((data))) {
						return;
					}
					...
					final IBotData botData = new PlayerBotData(botId, botName, botMotto, botFigure, botGender, client.getPlayer().getData().getUsername(), client.getPlayer().getId(), "", true, 7, type, mode, "");
					...
					if (songName != null && !songName.isEmpty()) {
						...
						if (musicData != null) {
							extraData = String.format("%s\n%s\n%s\n%s\n%s\n%s", client.getPlayer().getData().getUsername(), Calendar.getInstance().get(Calendar.YEAR), Calendar.getInstance().get(Calendar.MONTH), Calendar.getInstance().get(Calendar.DAY_OF_MONTH), musicData.lengthSeconds(), musicData.title());
						}
					}
				}
				...
				for (long newItem : newItems) {
					...
					if (giftData == null)
						unseenItems.add(client.getPlayer().getInventory().add(newItem, bundledItem.itemId(), extraData, giftData, item.getLimitedTotal() > 0 ? new LimitedEditionItemData(bundledItem.itemId(), item.getLimitedSells(), item.getLimitedTotal()) : null));
					
					if (isTeleport) teleportIds[newItems.indexOf(newItem)] = newItem;
				}
		...
		if (client != null) {
			...
			if (client.getPlayer() != null) {
				if (client.getPlayer().getInventory() != null) {
					unseenItems = newItems.stream().mapToLong(newItem -> newItem).mapToObj(newItem -> client.getPlayer().getInventory().add(newItem, ItemManager.getInstance().getBySpriteId(giftData.getSpriteId()).getId(), GiftData.EXTRA_DATA_HEADER + JsonUtil.getInstance().toJson(giftData), giftData, null)).collect(Collectors.toSet());
				}
	...
	public int applyDiscount(int cost, int quantity) {
		...
		if (quantity <= 5) {
			...
			discountMultiplier = (quantity - 11) / 2 + 2;
		} else if (quantity <= 22) {
			discountMultiplier = (quantity - 17) / 2 + 4;
		} else if (quantity <= 28) {
			discountMultiplier = (quantity - 23) / 2 + 6;
		} else if (quantity <= 34) {
			discountMultiplier = (quantity - 29) / 2 + 8;
		} else if (quantity <= 40) {
			discountMultiplier = (quantity - 35) / 2 + 10;
		} else if (quantity <= 46) {
			discountMultiplier = (quantity - 41) / 2 + 13;
		} else if (quantity <= 52) {
			discountMultiplier = (quantity - 47) / 2 + 15;
		} else if (quantity <= 59) {
			discountMultiplier = (quantity - 53) / 2 + 17;
		} else if (quantity <= 64) {
			discountMultiplier = (quantity - 60) / 2 + 20;
		} else if (quantity <= 70) {
			discountMultiplier = (quantity - 65) / 2 + 22;
		} else if (quantity <= 76) {
			discountMultiplier = (quantity - 71) / 2 + 24;
		} else if (quantity <= 82) {
			discountMultiplier = (quantity - 77) / 2 + 26;
		} else if (quantity <= 88) {
			discountMultiplier = (quantity - 83) / 2 + 28;
		} else if (quantity <= 94) {
			discountMultiplier = (quantity - 89) / 2 + 30;
		} else if (quantity <= 100) {
			discountMultiplier = (quantity - 95) / 2 + 32;
		}
