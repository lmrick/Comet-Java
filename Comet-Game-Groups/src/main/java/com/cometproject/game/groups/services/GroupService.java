...
public class GroupService implements IGroupService {
	...
	public GroupService(Cache<Integer, IGroup> groupCache, Cache<Integer, IGroupData> groupDataCache, IGroupItemService groupItemService, IGroupRepository groupRepository, IGroupMemberRepository groupMemberRepository, IGroupForumRepository groupForumRepository) {
	...
	public IGroupData getData(final int groupId) {
		...
		final DataWrapper<IGroupData> data = new DataWrapper<>();
	...
	public IGroup getGroup(final int groupId) {
		...
		final DataWrapper<List<IGroupMember>> groupMemberData = new DataWrapper<>();
		final DataWrapper<List<Integer>> requestsData = new DataWrapper<>();
		...
		if (!foundOwner) {
			this.groupMemberRepository.create(groupId, groupData.getOwnerId(), GroupAccessLevel.OWNER, (member) -> groupMemberData.get().add(member));
		}
	...
	public void addForum(IGroup group) {
		...
		DataWrapper<IForumSettings> forumSettingsData = new DataWrapper<>();
		groupForumRepository.getSettingsByGroupId(group.getId(), forumSettingsData::set);
		ForumComponent forumComponent = new ForumComponent(forumSettingsData.get(), pinnedThreads, forumThreads);
	...
	public void createRequest(IGroup group, int playerId) {
		if (group.getMembers().hasMembership(playerId) || group.getMembers().getMembershipRequests().contains(playerId)) {
			return;
		}
	...
	private IGroup build(List<IGroupMember> groupMemberData, List<Integer> requestsData, IGroupData groupData) {
		...
		groupMemberData.forEach(groupMember -> {
			if (groupMember.getAccessLevel().isAdmin()) administrators.add(groupMember.getPlayerId());
		...
		if (groupData.hasForum()) {
			final DataWrapper<IForumSettings> forumSettingsData = new DataWrapper<>();
			final DataWrapper<Map<Integer, IForumThread>> forumThreadData = new DataWrapper<>();
			final DataWrapper<List<Integer>> pinnedThreadData = new DataWrapper<>();
		...
		final IGroup group = this.groupFactory.createGroupInstance(groupData, groupMembers, requests, administrators, forumSettings, pinnedThreads, forumThreads);
