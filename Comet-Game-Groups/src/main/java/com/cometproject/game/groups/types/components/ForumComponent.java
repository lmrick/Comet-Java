...
public record ForumComponent(IForumSettings forumSettings, List<Integer> pinnedThreads, Map<Integer, IForumThread> forumThreads) implements IForumComponent {
	...
	public List<IForumThread> getForumThreads(int start) {
		...
		if (start == 0) {
			for (int pinnedThread : this.pinnedThreads) {
				IForumThread forumThread = this.forumThreads().get(pinnedThread);
